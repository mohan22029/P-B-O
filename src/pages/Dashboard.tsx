...
// These are simplified versions of what a library like shadcn/ui would provide.
    ...
    <div className="space-y-6 p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      <Card>
        ...
        <CardContent className="flex flex-col md:flex-row gap-4 items-center">
          <Select onChange={(e) => handleDrugSelection(e.target.value, 0)} value={selectedDrugs[0]}>
            <option value="">Choose Drug 1...</option>
            {allDrugs.map((d, i) => (
              <option key={`${d.drug_name}-1-${i}`} value={d.drug_name}>{d.drug_name}</option>
            ))}
          </Select>
          <Select onChange={(e) => handleDrugSelection(e.target.value, 1)} value={selectedDrugs[1]}>
            <option value="">Choose Drug 2 (Optional)...</option>
            {allDrugs.map((d, i) => (
              <option key={`${d.drug_name}-2-${i}`} value={d.drug_name}>{d.drug_name}</option>
            ))}
          </Select>
          <Button 
